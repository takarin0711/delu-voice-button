...
DELUTAYA（[@delutaya](https://www.youtube.com/@delutaya)）のボイスを楽しむプロジェクトページです。

**ウェブページ版**: https://takarin0711.github.io/delu-voice-button/

## 機能

### ウェブページ版
- ブラウザで直接アクセスして音声を再生できる
- 30種類の動物ボイス・16種類の笑いボイス・12種類の叫びボイスがグリッド表示される

<img width="844" height="781" alt="Image" src="https://github.com/user-attachments/assets/2d3c05fd-e714-4722-862f-15dae4daa78b" />

### Chrome拡張機能版 (メンテしてないので非推奨)
...
1. このリポジトリのファイルをダウンロードする
   ...
   **方法B：gitコマンドでクローン**
   ```
   git clone https://github.com/takarin0711/delu-voice-button.git
   ```
2. Chromeで `chrome://extensions` を開く
3. 右上の「デベロッパーモード」をオンにする
4. 「パッケージ化されていない拡張機能を読み込む」をクリックし、「delu-voice-button」フォルダを選択する

## 使い方

[ウェブページ版]
1. https://takarin0711.github.io/delu-voice-button/ を開く
...
├── sounds.js       # 動物ボイス定義（拡張機能・ウェブページ共用）
├── manifest.json   # 拡張機能の設定
├── content.js      # 拡張機能: ボタン注入・ナビゲーション管理・音声再生
├── styles.css      # 拡張機能: ボタン・ドロップダウンのスタイル
├── icon/           # アイコン画像
├── voice/
│   ├── animal/     # 動物ボイス音声ファイル（ウェブページ・拡張機能で共用）
│   │   ├── 01_duck.mp3
│   │   ├── 02_sheep.mp3
│   │   └── ...（30種類）
│   ├── laugh/      # 笑いボイス音声ファイル（ウェブページ版専用）
│   │   ├── 01.mp3
│   │   └── ...（16種類）
│   └── scream/     # 叫びボイス音声ファイル（ウェブページ版専用）
│       ├── 01.mp3
│       └── ...（12種類）
└── README.md
```
...
1. `voice/animal/` フォルダにmp3ファイルを追加する
...
const SOUNDS = [
  ...
  // 例: ここに追加
  { label: "🐙 タコ",   file: "voice/animal/31_octopus.mp3" },
];
```

### 笑いボイス（ウェブページ版専用）

1. `voice/laugh/` フォルダにmp3ファイルを追加する
2. `page.js` の `LAUGH_SOUNDS` 配列にエントリを追加する

```js
const LAUGH_SOUNDS = [
  { label: "笑い1",  file: "voice/laugh/01.mp3" },
  // 例: ここに追加
  { label: "笑い17", file: "voice/laugh/17.mp3" },
];
```

### 叫びボイス（ウェブページ版専用）

1. `voice/scream/` フォルダにmp3ファイルを追加する
2. `page.js` の `SCREAM_SOUNDS` 配列にエントリを追加する

```js
const SCREAM_SOUNDS = [
  { label: "叫び1",  file: "voice/scream/01.mp3" },
  // 例: ここに追加
  { label: "叫び13", file: "voice/scream/13.mp3" },
];
```
